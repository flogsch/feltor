...
/**
 * @brief A Helmholtz-type operator \f[ y = \exp^(\chi x) * (1 - \alpha M x) $\f]
 *
 * @ingroup matrixoperators
 *
 * where \f$ \chi\f$ is a vector and \f$\alpha\f$ a scalar and \f$F\f$ is an operator.
...
///@brief a 2d HelmholtzLN opereator \f$ (\chi . -ln(1-\alpha F .)) \f$
...
template< class M, class V>
struct TensorTraits< GeneralHelmholtzLN<M, V> >
{
    using value_type  = get_value_type<V>;
    using tensor_category = SelfMadeMatrixTag;
};
