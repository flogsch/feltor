...
struct Explicit
{
  ...
  private:
    ...
    Container m_phi, m_dxphi, m_dyphi, m_uex, m_uey, m_ustx, m_usty;
...
Explicit< Geometry, M, Container>::Explicit( const Geometry& grid, const Parameters& p ):
    ...
    m_phi( m_chi), m_dxphi(m_phi), m_dyphi( m_phi), m_uex(m_phi), m_uey(m_phi), m_ustx(m_phi), m_usty(m_phi),
        ...
        const Container & y, Container & yp)
{
    ...
    if (p.model == "standardCHM"){
        ...
        dg::blas2::symv( 1., m_centered[1], m_uE2, m_ustx); //compute advection velocities Ust
        dg::blas2::symv(-1., m_centered[0], m_uE2, m_usty);
